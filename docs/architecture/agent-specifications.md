...
- **`FindingRecord`** (a `findings[]` element, what `template_fill` requires): `{ id: string, finding_type: string, description: string, severity: "low"|"medium"|"high", deadline_days: int, rubric_version: string, citations: Citation[], quant_ref?: EdmQueryRef, status: "draft"|"signed_off"|"redrafted", model_confidence?: float }`.
...
- **`ComplianceVerdict`** (a `compliance_verdicts[]` element): `{ question_id: string, verdict: "compliant"|"non_compliant"|"insufficient_grounding", citation?: Citation, asserted_by: "model"|"human", model_confidence?: float }` — `asserted_by` is what lets a human-asserted verdict (decision 95) log distinctly from a model-asserted one, per `lfi-gap-analysis-resolution-v1`'s requirement. `model_confidence` (new, Principal Engineer audit 2026-09-18 #10) persists `supersession_graph.check()`'s `confidence` score onto the record it gated instead of discarding it once the `needs_human_review` routing decision is made — omitted when `asserted_by: "human"`, since a human assertion has no model confidence to report. Cheap to add now; a breaking-schema migration (ADR-0025) if deferred until real examinations are in flight.
...
- **Settings/Admin area** (outside any single examination): the severity/deadline rubric editor — **one shared, versioned rubric per license type** (resolves Principal Engineer audit 4.6's "global, per license type" ambiguity: "global" describes the *service* — one platform-wide service, not a per-examination copy — while "per license type" is its actual scoping dimension; v1 has exactly one license type, banks, per ADR-0002's lean-v1 scope, so this doesn't yet produce more than one rubric in practice, but the contract below is built to scope by it from the start rather than needing a breaking change the day a second license type ships) — Lead/Approver only, a simple current-state edit form (shows current version, save creates a new version; no dedicated version-history browser UI is built, but the edit form itself lists prior versions inline, sourced from `audit_log[]` entries filtered to `event_type: "rubric.version_changed"` — **resolves Principal Engineer audit 4.7**: rubric edits aren't scoped to any single examination, so "full history stays queryable via Audit Log" previously pointed Lead/Approver at a screen ADR-0029 scopes per-examination for that exact role, locking out the one person who edits the rubric; this inline list needs no separate portfolio-wide Audit Log access, since it's the rubric editor reading its own event-type slice of the same store, not a new screen) — and the LFI registry (**`LfiRegistryEntry` schema, resolves the registry half of Principal Engineer audit 6.4**: `{ lfi_id: string, name: string, license_type: string, sftp_root: string, contacts: [{ name: string, email: string }] }` — one row per LFI, independent of any examination; `sftp_root` here is the default the Setup sequence's `sftp_root_override` field falls back to when unset. This app owns this data directly, no external system-of-record integration exists to consume instead).
...
- **Open item**: `tool-contracts.md`'s `ocr_segment()` now gives this a provisional input/output/timeout/retry shape (resolves the contract half of Principal Engineer audit 6.4), but the underlying OCR product/vendor is still unpicked — no MCP servers identified yet, unlike the sanctions-screening gap in ADR-0028, which at least has real ones to point to.
