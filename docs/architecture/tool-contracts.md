...
**Used by**: Gap Analysis Agent — **the citation-grounding rule's tool** (ADR-0004).
...
**Output (error)**: `{ status: "error", reason: "corpus_unavailable" | "index_rebuilding" }` — **must never be treated as `found: false`.** Gap Analysis Agent's node must branch on `status` before it ever looks at `found`.
**Timeout**: 5s.
...
**Used by**: Gap Analysis Agent (context), Findings & Reporting Agent (quantitative track input).
...
**Used by**: Findings & Reporting Agent. **Shared, versioned platform service** (ADR-0020) — not owned by any one agent.
...
**Used by**: Findings & Reporting Agent. **Deterministic — code, not LLM** (ADR-0021).
...
**Used by**: Findings & Reporting Agent.
