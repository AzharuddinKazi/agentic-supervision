...
- Oracle: the "low hundreds of MB per examination, low-tens-of-GB/year" placeholder below assumed documents were referenced/metadata-tracked rather than stored as blobs. **ADR-0032 has since settled that question the other way** — the Fraud Database stores full document content, not pointers — so this number is now stale and needs to be re-derived from real per-document-size assumptions once the examiner team's actual document volume is known, not trusted as-is. Full document content is very likely the actual capacity bottleneck, not GPU sizing, once re-derived.
