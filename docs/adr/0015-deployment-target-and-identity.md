...
Both are worth recording because a future engineer might otherwise default to a specific platform's managed services (locking in primitives the on-prem decision doesn't support) or stand up a local user/password store (duplicating identity CBUAE already manages) — either would be a real regression from what's intended here.
