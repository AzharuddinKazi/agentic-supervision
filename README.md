...
- **`docs/architecture/lfi-guardrail-citation-grounding-v1.html`** — a UML-style sequence diagram for the single highest-risk interaction in the spec: Gap Analysis Agent's citation-grounding call flow, including the retry-vs-real-absence distinction (ADR-0004/ADR-0018) that a tool timeout must never collapse into.
