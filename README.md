...
- **`docs/decision-log.md`** — every question raised and decision made, in order (138 decisions across 22 rounds as of 2026-09-18), cross-referenced to ADRs. This is the committed, durable record — check it before reopening anything that looks settled, so a prior decision doesn't get silently re-litigated.
...
As of 2026-09-19, every ADR in `docs/adr/` follows the [Michael Nygard template](https://architecture-decision-record.github.io/templates/decision-record-template-by-michael-nygard/): `# Title`, then `## Status`, `## Context`, `## Decision`, `## Consequences`. (ADRs 0001–0032 were originally written in a lighter prose-only house style with no fixed sections — all 32 were reformatted into this structure on 2026-09-19 with no content lost, just reorganized under headers.) `Status` states acceptance date and, where applicable, supersession by a later ADR (e.g. ADR-0002's Status notes it was partially superseded by ADR-0010) — cross-check `docs/decision-log.md` too, since not every amendment is reflected in an ADR's own Status line historically. New ADRs should be written directly in this format going forward.
